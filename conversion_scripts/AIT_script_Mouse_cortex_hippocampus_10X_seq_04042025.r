...
# This script describes how we created an AIT taxonomy (scrattch v1.1) of the taxonomy for "Whole Cortex & Hippocampus - 10x genomics (2020) with SMART-seq taxonomy (2021)" (https://portal.brain-map.org/atlases-and-data/rnaseq/mouse-whole-cortex-and-hippocampus-smart-seq).
...
# ***NOTE***: The published dendrogram is not a binary tree and therefore is incompatible with scrattch.taxonomy at this time. Therefore we are creating a new tree for the AIT file.  Note that this tree will also NOT match the tree created in Mouse_cortex_hippocampus_SMART_seq_04042025

# ***NOTE***: There are 107 cells missing in the metadata that are therefore excluded from the counts and UMAP.

# ***NOTE***: MapMyCells Mapping failed for this one as well.  I do not know why.

## Read in the metadata
sampInfo <- as.data.frame(as.matrix(fread("metadata.csv"),rownames=1))

## Read in the data from hdf5
genes   <- rhdf5::h5read("expression_matrix.hdf5","/data/gene")
samples <- rhdf5::h5read("expression_matrix.hdf5","/data/samples")
print(".... reading in data from hdf5 file, which is quite slow.")
counts  <- rhdf5::h5read("expression_matrix.hdf5", "/data/counts")
rownames(counts) <- as.character(samples)
colnames(counts) <- as.character(genes)
counts  <- as(counts, "dgCMatrix") # Let's make this sparse right now.
taxonomy.counts = counts[rownames(sampInfo),]
...
omit <- c("cell_type_alt_alias","injection_type","platform","injection_roi","injection_materials","injection_method","cortical_layer")
taxonomy.metadata <- taxonomy.metadata[,!is.element(colnames(taxonomy.metadata),omit)]
...
## -- NOTE: We exclude neighborhood from the hierarchy for consistency with the SMART-seq file, but mapmycells still cannot be set up correctly (I don't know why...)
...
ensembl_id <- geneSymbolToEnsembl(gene.symbols = genes, ncbi.taxid = 10090)
...
## Subsample down to 1000 cells per cluster since this data set it too big to read in AIT
subsample <- subsampleCells(taxonomy.anno$cluster_id, 1000, seed=42)
...
AIT.anndata = buildTaxonomy(title="Mouse_cortex_hippocampus_10X_seq_04042025",
                            meta.data = taxonomy.anno[subsample,],
                            hierarchy = hierarchy,
                            ## --- Optional parameters ---
                            counts = taxonomy.counts[subsample,],
                            ...
                            embeddings = umap[subsample,], # Use pre-existing UMAP coordinates
                            ##
							...
							addMapMyCells = TRUE, 
                            ##
                            subsample=1000000)  ## Using a huge number since we don't want to subsample additionally
...
## The file created above is now uploaded to AWS and can be downloaded using the URL https://allen-cell-type-taxonomies.s3.us-west-2.amazonaws.com/Mouse_cortex_hippocampus_10X_seq_04042025.h5ad
...
# AIT.anndata <- loadTaxonomy("Mouse_cortex_hippocampus_10X_seq_04042025.h5ad")
