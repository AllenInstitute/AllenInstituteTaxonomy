...
# This script describes how we created an AIT taxonomy (scrattch v1.1) of the taxonomy for "Macaque Comparative LGN (2018)"  (https://portal.brain-map.org/atlases-and-data/rnaseq/comparative-lgn).
...
# *** NOTE ***: We are not using the geneInfo table as it is a different size from the counts tables.

## Read in the data
exons    <- as.matrix(fread("macaque_LGN_2021_exon-matrix.csv"),rownames=1)
introns  <- as.matrix(fread("macaque_LGN_2021_intron-matrix.csv"),rownames=1)
#geneInfo <- read.csv("macaque_LGN_2021_genes-rows.csv",row.names=1)
sampInfo <- read.csv("macaque_LGN_2021_metadata.csv",row.names=2)
...
cn <- (!(grepl("_color",cn)|grepl("_id",cn)|is.element(cn,c("X","fastq_file_name","dissection_roi","species"))))|(cn=="sample_id")
...
ensembl_id <- geneSymbolToEnsembl(gene.symbols = rownames(taxonomy.counts), ncbi.taxid = 9544)
...
colnames(taxonomy.metadata)[colnames(taxonomy.metadata)=="age"]     = "donor_age"
...
taxonomy.metadata$organism        = "Macaca nemestrina" 
...
roi    = c("Macaque Pulvinar - Inferior","Macaque LGN Magnocellular","Macaque LGN Parvocellular")
name   = c("inferior pulvinar nucleus",
...
uberon = c("UBERON:0002982","UBERON:0013606","UBERON:0002479")
...
AIT.anndata = buildTaxonomy(title="Macaque_LGN_SMART_seq_04042025",
...
## The file created above is now uploaded to AWS and can be downloaded using the URL https://allen-cell-type-taxonomies.s3.us-west-2.amazonaws.com/Macaque_LGN_SMART_seq_04042025.h5ad
...
# AIT.anndata <- loadTaxonomy("Macaque_LGN_SMART_seq_04042025.h5ad")
